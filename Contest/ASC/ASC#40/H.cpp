...
// Version     : 
// Copyright   : ASC #40 H
// Description : hash...pretty violent... Save the path and use bignum to solve
//============================================================================
...
bool dfs(int cur)
{
	...
	tmp.val = tmp.val * a[cur];
	tmp.vis[cur] = 1;
	for(int i = cur + 1 ; i <= n; i++)
	{
		 if( !dfs(i) ){ tmp.val = tmp.val / a[i]; tmp.vis[i] = 0; }
	}
	if(A.val < tmp.val)
	{
		A.val = tmp.val;
...
int main(){
	...
	long double ans[31];
	for(int i = 0; i <= 30; i++) ans[i] = 1;
	for(int i = 1; i <= 30; i++)
		ans[i] = ans[i-1] * (long double)a[i];
	for(int i = 1; i <= 30; i++)
		ans[i] = sqrt(ans[i]);
	string ans20[31];
	ans20[20] = "23619540863730";
	ans20[21] = "201813981102615";
	ans20[22] = "1793779293633437";
	ans20[23] = "16342050964565645";
	ans20[24] = "154170926013430326";
	ans20[25] = "1518409177581024365";
	ans20[26] = "15259825120722837478";
	ans20[27] = "154870329715038713659";
	ans20[28] = "1601991088154989174258";
	ans20[29] = "16725278985898957454695";
	ans20[30] = "177792163538134124432895";
	int T= 0;
	while( cin >> n && n != 0 )
	{
		cout << "Case #"<< ++T <<": " ;
